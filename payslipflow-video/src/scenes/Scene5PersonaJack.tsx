...
export const Scene5PersonaJack: React.FC = () => {
  ...
  // Thumbs up right after the payslips finish delivering -- the "job done"
  // beat, timed to land just after the fly animation above completes.
  const gestureProgress = interpolate(frame, [390, 420], [0, 1], {
    ...
    <AbsoluteFill style={{ background: COLORS.mist }}>
      <AbsoluteFill style={{ alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
        <div style={{ transform: `scale(${avatarIn})` }}>
          <PersonIllustration variant="jack" gestureProgress={gestureProgress} />
        </div>
