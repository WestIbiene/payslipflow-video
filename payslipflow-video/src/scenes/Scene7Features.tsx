...
// 12s scene, 5 features, ~72 frames (2.4s) each -- fast-cut montage feel.
const FEATURES = [
  ...
  { label: 'Scheduled sends', icon: 'clock' as const }
];
...
const ICONS: Record<string, React.ReactNode> = {
  ...
  clock: (
    <>
      <circle cx="12" cy="12" r="9" stroke={COLORS.navy} strokeWidth="1.6" />
      <path d="M12 7v5l3.5 3.5" stroke={COLORS.teal} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
    </>
  )
};
...
export const Scene7Features: React.FC = () => {
  ...
  const perFeature = 72; // 2.4s each x 5 = 12s
        ...
        {FEATURES.map((f, i) => {
          ...
          return (
            ...
            >
              ...
              <p style={{ fontFamily: FONT_FAMILY, fontWeight: 700, fontSize: 17, color: COLORS.navy, textAlign: 'center', maxWidth: 120 }}>
                {f.label}
              </p>
            </div>
          );
