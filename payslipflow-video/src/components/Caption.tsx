...
export const Caption: React.FC<{ text: string; sinceFrame?: number; dark?: boolean }> = ({
  text,
  sinceFrame = 0,
  dark = false
}) => {
  ...
  // Fades in relative to when THIS beat started, not the scene's start --
  // so multi-beat captions (persona scenes) re-fade on every line change.
  const localFrame = frame - sinceFrame;
